...
¦ (command) spam, too.                    								¦
¦ If JunkBuster kicks/bans too much innocent players, disable the Codigos ¦
...
Changelog Update 10:
	...
	- Command /jbreportes for all admins: Command lists the newest JB reports
	- Memory usage optimized
	- SetJBVar, GetJBVar functions
	- Warnings for drive-byers
	- Unified prefix syntax (CALLBACK:, PRIVATE:, PUBLIC:)
	- Optimized Codigo for IP-bans, blacklist, whitelist and tempbans
	...
	- Dialog for hack Codigos (If you don't understand you will after reading the dialog)
	- Colored variable list dialog (green = on, red = off)
	- AdvertisementCheck reCodigod
...
The file system is limitted by MAX_JB_KickS. Database isn't. Oh, before I forget, the database does also not work correctly
...
#define DIALOG_HACKCodigoS      (28351)
...
#define DIALOG_KICK 			(28356)
#define DIALOG_BAN 			    (28356)
...
//=Colores.
#define COL_ADM			     	    "{F63535}"
#define COL_GENERAL					"{2961B2}"
#define COL_GENERAL2				"{2961B2}"
...
#define COL_HOUSET					"{FF0000}"
#define COL_BLUE					"{2641FE}"
#define COL_DUDA					"{FF4719}"
#define COL_REPO					"{FFCABC}"
#define COL_PINK					"{F781F3}"

#define COLOR_GENERAL       0x00A2FFFF
#define COLOR_C_PUNTO       0xF5CB38FF
#define COLOR_AZUL          0x23A9D6FF
#define COLOR_VIP 			0xC93CCE00
#define COLOR_FBI 			0x8D8DFF00
#define COLOR_CNN 			0x049C7100
#define COLOR_MEDICO 		0xFF828200
#define COLOR_GUARDIA 		0x9ACD3200
#define COLOR_TAXI 			0xF2FF0000
...
#define COLOR_ORANGE 		0x23A9D6FF
...
enum JB_pInfo
{
	...
	JB_pAirbraking,
	Float: JB_pOldAirbrakePos [3],
	JB_pLastAirbrakeSpeed,
	JB_pLastAirbrakeCheck,
	JB_pWallriding,
	JB_pLastCheck,
	JB_pLastDrunkLevel,
	...
	Float: JB_pOnFootPos [3],
	Float: JB_pAFKPos [3],
	Float: JB_pLastKeyPressed,
	BitArray: JB_pWeaponForbidden <MAX_WEAPONS>,
	...
	JB_pLastVehicle,
	JB_pFreezeTime,
	Float: JB_pVehicleHealth,
	JB_pLastMoneyChange,
	JB_pLastLostMoney,
	JB_pLastBoughtWeapon [2],
	JB_pBuyingGuns
};
...
static
	JB_PlayerInfo [MAX_PLAYERS][JB_pInfo],
	JB_PlayerWeaponAmmo [MAX_PLAYERS][MAX_WEAPONS],
	JB_PlayerWeapons [MAX_PLAYERS][MAX_WEAPON_SLOTS char],
	JB_SyncInfo [MAX_PLAYERS][MAX_SYNC_TYPES][JB_sInfo],
	BitArray: JB_Freezed <MAX_PLAYERS>,
	BitArray: JB_KickBan <MAX_PLAYERS>,
	BitArray: JB_FullyConnected <MAX_PLAYERS>,
	BitArray: JB_AntiBugKilled <MAX_PLAYERS>,
	BitArray: JB_Spectating <MAX_PLAYERS>,
	BitArray: JB_BuyingInAmmuNation <MAX_PLAYERS>,
	BitArray: JB_GunBought <MAX_PLAYERS>,
	JB_PlayerClassWeapons [MAX_CLASSES][3][2],
	JB_SpawnWeapons [MAX_PLAYERS][3][2],
	bool: JB_PlayerPedAnims = false,
	JB_Homepage [32],
	Text: JB_KickBanTitle = Text: INVALID_TEXT_DRAW,
	Text: JB_KickBanInfo = Text: INVALID_TEXT_DRAW,
	Text: JB_KickBanHelp = Text: INVALID_TEXT_DRAW,
	JB_PickupType [MAX_PICKUPS char] = {PICKUP_TYPE_NONE, ...},
	JB_PickupVar [MAX_PICKUPS][2],
	Float: JB_PickupPos [MAX_PICKUPS][3],
	BitArray: JB_StaticPickup <MAX_PICKUPS>, // Simulate a static pickup...
	JB_PickupList [MAX_PICKUPS],
	JB_PickupCount,
	JB_Reports [MAX_REPORTS][128 char],
	JB_ReportIndex,
	JB_ReportCount,
	JB_Warnings [MAX_PLAYERS][MAX_JB_VARIABLES char],
	JB_Variables [MAX_JB_VARIABLES],
	BadWords [MAX_BAD_WORDS][32],
	BadWordsCount,
	ForbiddenWeapons [MAX_FORBIDDEN_WEAPONS char],
	ForbiddenWeaponsCount,
	JB_VehicleComponents [MAX_VEHICLES][MAX_COMPONENT_SLOTS char], // Save memory by just adding 999 to the byte value when accessing.
	...
	Float: JB_VehiclePos [MAX_VEHICLES][3],
	JB_AirbrakeDetection;
	...
	static
		TempBanInfo [MAX_JB_KickS][tbInfo],
		TempBanCount,
		Blacklist [MAX_JB_KickS][MAX_PLAYER_NAME char],
		BlacklistCount,
		Whitelist [MAX_JB_KickS][MAX_PLAYER_NAME char],
		WhitelistCount,
		IpBans [MAX_JB_KickS],
		IpBanCount;
...
static const JB_VariableNames [MAX_JB_VARIABLES][] =
{
	"WeaponHack",
	"MoneyHack",
	"Jetpack",
...
static const JB_VarDescription [MAX_JB_VARIABLES][] =
{
...
static const Float: JB_PayNSpray [][3] =
{
...
static const JB_VehicleInfo [][JB_vInfo] =
{
	{!"Landstalker", 140},
	{!"Bravura", 131},
	{!"Buffalo", 166},
...
enum JB_SAZONE_MAIN
{ //Betamaster
	JB_SAZONE_NAME [28 char],
	Float: JB_SAZONE_AREA [6]
};

static const JB_SAZones [][JB_SAZONE_MAIN] =
...
forward ServerDataUpdate();
PUBLIC: ServerDataUpdate()
{

    
    new RandomGo = random(2);
    switch(RandomGo) {
        case 0: SendRconCommand("hostname [ESP] SouthCenter - Juego de Rol");
        case 1: SendRconCommand("hostname [SC-RP] Agrega: samp.southcenter.es:7777");
        case 2: SendRconCommand("hostname [SC-RP] Agrega: noip.southcenter.es:7777");
    }
    return 1;
}
...
PUBLIC: AddWeaponPickup (Float: x, Float: y, Float: z, weaponid, ammo, worldid)
{
	...
	if (pickupid != -1)
	{
		...
		DOB::SetBit (JB_StaticPickup, pickupid, false);
		if (JB_PickupCount < MAX_PICKUPS)
			JB_PickupList [JB_PickupCount++] = pickupid;
		JB_PickupPos [pickupid][0] = x;
		JB_PickupPos [pickupid][1] = y;
		JB_PickupPos [pickupid][2] = z;
	}
	return pickupid;
}
		...
		switch (model)
		{
			case 1240: //Health
			{
				...
				if (pickupid != -1)
				{
					...
					DOB::SetBit (JB_StaticPickup, pickupid, true);
					if (JB_PickupCount < MAX_PICKUPS)
						JB_PickupList [JB_PickupCount++] = pickupid;
					JB_PickupPos [pickupid][0] = x;
					JB_PickupPos [pickupid][1] = y;
...
{
	if (pickupid >= 0 && pickupid < MAX_PICKUPS && !DOB::GetBit (JB_StaticPickup, pickupid) && DestroyPickup (pickupid))
	{
        DOB::SetBit (JB_StaticPickup, pickupid, false);
        for (new i = 0; i < JB_PickupCount; ++i)
        {
            if (JB_PickupList [i] == pickupid)
            {
                JB_PickupList [i] = JB_PickupList [--JB_PickupCount];
                break;
            }
        }
...
PUBLIC: JB_SetVehicleVelocity (vehicleid, Float: x, Float: y, Float: z)
{
	if (vehicleid != INVALID_VEHICLE_ID)
	{
		...
		{
			JB_LogEx ("No se pudo establecer la velocidad para el vehículo %d porque la velocidad máxima es %d KM/H. (%d KM/H bloqueado)", vehicleid, JB_Variables [MAX_SPEED], tspeed);
			return 0;
		}
...
PUBLIC: JB_PutPlayerInVehicle (playerid, vehicleid, seatid)
{
	if (IsPlayerConnected (playerid) && vehicleid != INVALID_VEHICLE_ID)
	{
		if (PutPlayerInVehicle (playerid, vehicleid, seatid))
		{
			...
			JB_PlayerInfo [playerid][JB_pLastVehicle] = vehicleid;
...
PUBLIC: JB_GetPlayerMoney (playerid)
{
	if (IsPlayerConnected (playerid))
	{
	    if (JB_Variables [MONEY_HACK])
	        return min (GetPlayerMoney (playerid), JB_PlayerInfo [playerid][JB_pMoney]);
		else
...
PUBLIC: JB_SetPlayerPosFindZ (playerid, Float: x, Float: y, Float: z)
{
	if (IsPlayerConnected (playerid))
	{
...
PUBLIC: JB_SetVehiclePos (vehicleid, Float: x, Float: y, Float: z)
{
	if (vehicleid != INVALID_VEHICLE_ID)
	{
	    JB_VehiclePos [vehicleid][0] = x;
	    JB_VehiclePos [vehicleid][1] = y;
	    JB_VehiclePos [vehicleid][2] = z;
...
PUBLIC: JB_GivePlayerWeapon (playerid, weaponid, ammo)
{
	...
	{
	    JB_PlayerInfo [playerid][JB_pLastBoughtWeapon] = 0;
...
PRIVATE: SyncWeapons_CS (playerid, bool: buyableonly = false)//ClientSide
{
	if (IsPlayerConnected (playerid))
	{
		...
		for (i = 0; i < MAX_WEAPON_SLOTS; ++i)
		{
			...
			{
				JB_PlayerWeapons [playerid]{i} = weapons;
...
PUBLIC: JB_CreateVehicle (vehicletype, Float: x, Float: y, Float: z, Float: rotation, color1, color2, respawn_delay)
{
	...
	if (vehicleid != INVALID_VEHICLE_ID)
	{
	    ...
	    for (new i = 0; i < MAX_COMPONENT_SLOTS; ++i)
	    	JB_VehicleComponents [vehicleid]{i} = 0;
	}
	return vehicleid;
}

PUBLIC: JB_AddStaticVehicle (modelid, Float: spawn_x, Float: spawn_y, Float: spawn_z, Float: z_angle, color1, color2)
{
	new vehicleid = AddStaticVehicle (modelid, spawn_x, spawn_y, spawn_z, z_angle, color1, color2);
    if (vehicleid != INVALID_VEHICLE_ID)
	{
	    JB_VehiclePos [vehicleid][0] = spawn_x;
	    JB_VehiclePos [vehicleid][1] = spawn_y;
	    JB_VehiclePos [vehicleid][2] = spawn_z;
	    for (new i = 0; i < MAX_COMPONENT_SLOTS; ++i)
	    	JB_VehicleComponents [vehicleid]{i} = 0;
	}
	return vehicleid;
...
PUBLIC: JB_AddStaticVehicleEx (modelid, Float: spawn_x, Float: spawn_y, Float: spawn_z, Float: z_angle, color1, color2, respawn_delay)
{
...
PUBLIC: JB_AddVehicleComponent (vehicleid, componentid)
{
	new slot = GetVehicleComponentType (componentid);
	if (slot != -1)
	{
	    AddVehicleComponent (vehicleid, componentid);
	    JB_VehicleComponents [vehicleid]{slot} = componentid - 999;
	    return 1;
	}
	return 0;
}

PUBLIC: JB_RemoveVehicleComponent (vehicleid, componentid)
{
	new slot = GetVehicleComponentType (componentid);
	if (slot != -1 && GetVehicleComponentInSlot (vehicleid, slot) == componentid)
	{
	    RemoveVehicleComponent (vehicleid, componentid);
	    JB_VehicleComponents [vehicleid]{slot} = 0;
	    return 1;
	}
	return 0;
}

PUBLIC: JB_RepairVehicle (vehicleid)
{
    if (vehicleid == INVALID_VEHICLE_ID)
	    return 0;
    ...
    ForEachPlayer (i)
	{
	    if (GetPlayerVehicleID (i) == vehicleid)
	    {
	        JB_PlayerInfo [i][JB_pVehicleHealth] = 1000.0;
...
PUBLIC: JB_SetVehicleHealth (vehicleid, Float: health)
{
	...
	ForEachPlayer (i)
	{
	    if (GetPlayerVehicleID (i) == vehicleid)
	    {
	        JB_PlayerInfo [i][JB_pVehicleHealth] = tmp_health;
	        SetSyncTime (i, SYNC_TYPE_VEHICLE);
		}
	}
...
{
	DOB::SetBit (JB_Freezed, playerid, toggle != 0);
	if (!toggle && IsPlayerInAnyVehicle (playerid))
		SetSyncTime (playerid, SYNC_TYPE_VEHICLE);
	return TogglePlayerControllable (playerid, toggle);
}

PUBLIC: JB_TogglePlayerSpectating (playerid, toggle)
{
	DOB::SetBit (JB_Spectating, playerid, toggle != 0);
	return TogglePlayerSpectating (playerid, toggle);
}

PUBLIC: JB_SetPlayerSpawnKillProtected (playerid, set)
{
	if (IsPlayerConnected (playerid))
	{
		if (set)
...
{
	if (JB_PlayerInfo [playerid][JB_pCurrentState] == PLAYER_STATE_ONFOOT && GetPlayerWeapon (playerid) == 26
		&& HasTimePassed (JB_PlayerInfo [playerid][JB_pLastSawnOffShot], 10000)
		&& !HasTimePassed (JB_PlayerInfo [playerid][JB_pLastUpdate], 1000))
	{
		JB_Kick (playerid, "Not reloading (Sawn-off Shotgun) [Codigo 1]");
		return 1;
	}
	return 0;
}

PUBLIC: JB_AntiBugKill (playerid)
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid) && IsPlayerInValidState (playerid) && !DOB::GetBit (JB_AntiBugKilled, playerid)) //!JB_PlayerInfo [playerid][JB_pAntiBugKilled])
	{
		new
			Float: x,
			Float: y,
		...
		DOB::SetBit (JB_AntiBugKilled, playerid, true);
...
PRIVATE: JB_MutePlayer (playerid, time, reason [])//Time is seconds.
{
	if (IsPlayerConnected (playerid) && time)
	{
		...
		JB_SendFormattedMessageToAll (JB_RED, ""COL_ADM"Administración:"COL_WHITE" %s fue sileciado durante %d segundo(s) por JunkBuster, razón: %s", JB_ReturnPlayerName(playerid), playerid, time, reason);
		return 1;
	}
	return 0;
}

PRIVATE: JB_Kick (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
	    
	    new string[128];
	    JB_SendFormattedMessage (playerid, JB_RED, ""COL_ADM"Administración:"COL_WHITE" JunkBuster te ha expulsado del servidor, razón: %s", JB_ReturnPlayerName(playerid), reason);
	    format(string,sizeof(string),"~r~Expulsado por JunkBuster, razon:~n~%s",reason);
	    GameTextForPlayer(playerid,string,60000,4);

	    TogglePlayerControllable(playerid,false);
	    JB_SendFormattedMessageToAll(JB_RED,""COL_ADM"Administración:"COL_WHITE" %s fue expulsado por JunkBuster, razón: %s",JB_ReturnPlayerName(playerid),reason);
...
PRIVATE: JB_Ban (playerid, reason [])
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid))
	{
		JB_SendFormattedMessage (playerid, JB_RED, ""COL_ADM"Administración:"COL_WHITE" JunkBuster te ha baneado del servidor, razón: %s", reason);
		TogglePlayerControllable (playerid, false);
		SetCameraBehindPlayer (playerid);
		JB_SendFormattedMessageToAll (JB_RED, ""COL_ADM"Administración:"COL_WHITE" %s fue baneado por JunkBuster, razón: %s", JB_ReturnPlayerName(playerid), reason);
		...
		SetTimerEx("TimerBAN",500,false,"i",playerid);
		return 1;
	}
	return 0;
}
...
PRIVATE: DisplayKickBanText (playerid, reason [], bool: ban)
{
	if (IsPlayerConnected (playerid))
	{
		...
		if (JB_KickBanTitle == Text: INVALID_TEXT_DRAW)
		{
	        JB_KickBanTitle = TextDrawCreate (320.000000, 100.000000, ban ? ("JunkBuster~n~Informacion de baneo~n~~n~~n~~n~~n~~n~~n~~n~~n~") : ("JunkBuster~n~Informacion de expulsion~n~~n~~n~~n~~n~~n~~n~~n~~n~"));
			TextDrawAlignment (JB_KickBanTitle, 2);
			TextDrawBackgroundColor (JB_KickBanTitle, 255);
			TextDrawFont (JB_KickBanTitle, 2);
			TextDrawLetterSize (JB_KickBanTitle, 0.500000, 2.999999);
			TextDrawColor (JB_KickBanTitle, -16776961);
			TextDrawSetOutline (JB_KickBanTitle, 1);
			TextDrawSetProportional (JB_KickBanTitle, 1);
			TextDrawUseBox (JB_KickBanTitle, 1);
			TextDrawBoxColor (JB_KickBanTitle, 96);
			TextDrawTextSize (JB_KickBanTitle, 7.000000, 240.000000);
		}
		else
		    TextDrawSetString (JB_KickBanTitle, ban ? ("JunkBuster~n~Informacion de baneo~n~~n~~n~~n~~n~~n~~n~~n~~n~") : ("JunkBuster~n~Informacion de expulsion~n~~n~~n~~n~~n~~n~~n~~n~~n~"));
		TextDrawShowForPlayer (playerid, JB_KickBanTitle);

		if (JB_KickBanInfo == Text: INVALID_TEXT_DRAW)
		{
		    JB_KickBanInfo = TextDrawCreate (202.000000, 155.000000, "_");
			TextDrawBackgroundColor (JB_KickBanInfo, 255);
			TextDrawFont (JB_KickBanInfo, 1);
			TextDrawLetterSize (JB_KickBanInfo, 0.419999, 1.899999);
			TextDrawColor (JB_KickBanInfo, -1);
			TextDrawSetOutline (JB_KickBanInfo, 0);
			TextDrawSetProportional (JB_KickBanInfo, 1);
			TextDrawSetShadow (JB_KickBanInfo, 1);
		}
		format (string, sizeof (string), "~y~Nombre:~n~~w~%s~n~~y~Fecha:~n~~w~%s~n~~y~Tiempo:~n~~w~%s~n~~y~IP:~n~~w~%s~n~~y~Razon:~n~~w~%s", JB_ReturnPlayerName(playerid), JB_GetDate (), JB_GetTime (),
			JB_ReturnPlayerIp(playerid), reason);
		TextDrawSetString (JB_KickBanInfo, string);
		TextDrawShowForPlayer (playerid, JB_KickBanInfo);

		if (JB_KickBanHelp == Text: INVALID_TEXT_DRAW)
		{
		    JB_KickBanHelp = TextDrawCreate (320.000000, 330.000000, "_");
			TextDrawAlignment (JB_KickBanHelp, 2);
			TextDrawBackgroundColor (JB_KickBanHelp, 255);
			TextDrawFont (JB_KickBanHelp, 1);
			TextDrawLetterSize (JB_KickBanHelp, 0.319999, 1.399999);
			TextDrawColor (JB_KickBanHelp, -1);
			TextDrawSetOutline (JB_KickBanHelp, 0);
			TextDrawSetProportional (JB_KickBanHelp, 1);
			TextDrawSetShadow (JB_KickBanHelp, 1);
			TextDrawUseBox (JB_KickBanHelp, 1);
			TextDrawBoxColor (JB_KickBanHelp, 0);
			TextDrawTextSize (JB_KickBanHelp, 2.000000, 243.000000);
		}
		format (string, sizeof (string), "~g~Si has sido equivocadamente %s, por favor presiona ~b~F8~g~ para hacer una captura de pantalla y quejarte en ~p~%s~g~.", ban ? ("baneado") : ("expulsado"), JB_Homepage);
		TextDrawSetString (JB_KickBanHelp, string);
		TextDrawShowForPlayer (playerid, JB_KickBanHelp);
		return 1;
	}
	return 0;
}
...
PUBLIC: JunkBusterChrome (playerid, comment [])
{
	if (IsPlayerConnected (playerid))
	{
		JB_LogEx ("Ejecutando JunkBuster Chrome para el usuario '%s'.", JB_ReturnPlayerName(playerid));
		new
			string [256],
			string2 [64],
			Float: fvar [9],
		...
		format (string, sizeof (string), "[%s, %d]\r\nComentario: %s\r\nFecha: %s\r\nTiempo: %s\r\n", JB_ReturnPlayerName(playerid), playerid, comment, JB_GetDate (), JB_GetTime ());
		fwrite (f, string);
		format (string, sizeof (string), "IP: %s\r\nPing: %d\r\n", JB_ReturnPlayerIp(playerid), playerid, GetPlayerPing (playerid));
		fwrite (f, string);
		#if defined gpci
...
{
	for (new i = 0; i < JB_PickupCount; ++i)
	    if (IsPlayerInRangeOfPoint (playerid, 7.0, JB_PickupPos [JB_PickupList [i]][0], JB_PickupPos [JB_PickupList [i]][1], JB_PickupPos [JB_PickupList [i]][2]))
	        return true;
	return false;
}
...
PRIVATE: IsArmedVehicle (vehicleid)
{
	...
	for (new i = 0; i < sizeof (JB_ArmedVehicles); ++i)
	    if (m == JB_ArmedVehicles [i])
	        return true;
	return false;
}
...
{
    for (new i = 0; i < sizeof (JB_PayNSpray); ++i)
		if (IsPlayerInRangeOfPoint (playerid, 10.0, JB_PayNSpray [i][0], JB_PayNSpray [i][1], JB_PayNSpray [i][2]))
			return true;
	return false;
}
...
PRIVATE: IsPlayerNearVendingMachine (playerid)
{
	for (new i = 0; i < sizeof (JB_VendingMachines); ++i)
		if (IsPlayerInRangeOfPoint (playerid, 2.5, JB_VendingMachines [i][0], JB_VendingMachines [i][1], JB_VendingMachines [i][2]))
			return true;
	return false;
}
...
{
	if (JB_PlayerInfo [playerid][JB_pCurrentState] > 0 && JB_PlayerInfo [playerid][JB_pCurrentState] <= 6)
		return true;
	return false;
}
...
PRIVATE: SetSyncTime (playerid, synctype, base = 3)
{
    ...
    JB_SyncInfo [playerid][synctype][JB_sSyncTime] = base;
	JB_SyncInfo [playerid][synctype][JB_sLastSyncUpdate] = GetTickCount ();
	return 1;
}
...
PRIVATE: BadWordsCheck (text [])
{
	/*
		...
		This Codigo will prevent this word in all forms:
		 - "no.ob"
		 - "no	 ob"
...
PUBLIC: CheckText (playerid, text []) //return 1: something forbidden found, return 0: everything OK!
{
	...
	{
		SendClientMessage (playerid, JB_RED, "JunkBuster:{FFFF00} You are not allowed to use Caps Lock! Press [Caps Lock] to disable it.");
		return 1;
	}
	...
	{
		SendClientMessage (playerid, JB_RED, "JunkBuster:{FFFF00} You are not allowed to use such words!");
		return 1;
	}
	return 0;
}
...
PUBLIC: IsWeaponForbiddenForPlayer (playerid, weaponid)
{
	...
	return (DOB::GetBit (JB_PlayerInfo [playerid][JB_pWeaponForbidden], weaponid) /*JB_PlayerInfo [playerid][JB_pWeaponForbidden][weaponid]*/ || (IsForbiddenWeapon (weaponid) && DOB::GetBit (JB_PlayerInfo [playerid][JB_pWeaponForbidden], weaponid))); //JB_PlayerInfo [playerid][JB_pWeaponForbidden][weaponid]));
}
...
PUBLIC: AllowWeaponForPlayer (playerid, weaponid)
{
	...
	if (IsPlayerConnected (playerid))
	{
	    DOB::SetBit (JB_PlayerInfo [playerid][JB_pWeaponForbidden], weaponid, false);
...
PUBLIC: ForbidWeaponForPlayer (playerid, weaponid, antibugkill)
{
	...
	if (IsPlayerConnected (playerid))
	{
		new tmpforbid = DOB::GetBit (JB_PlayerInfo [playerid][JB_pWeaponForbidden], weaponid); //JB_PlayerInfo [playerid][JB_pWeaponForbidden][weaponid];
			
		DOB::SetBit (JB_PlayerInfo [playerid][JB_pWeaponForbidden], weaponid, true);
		...
		if (!tmpforbid && JB_Variables [ANTI_BUG_KILL] && antibugkill)//Wasn't forbidden before and is now forbidden.
			JB_AntiBugKill (playerid); //forbidding a minigun when leaving for example a minigun deathmatch may result in a ban when this isn't used.
		return 1;
	}
	return 0;
}
...
PRIVATE: JunkBusterReport (playerid, report [], details [])
{
	if (IsPlayerConnected (playerid))
	{
		...
		format (string, sizeof (string), "[%s] %s (%d, %s) con %s, %s", JB_GetTime (), JB_ReturnPlayerName(playerid), playerid, JB_ReturnPlayerIp(playerid), report, details);
        strpack (JB_Reports [JB_ReportIndex], string);
		JB_ReportIndex = (JB_ReportIndex + 1) % MAX_REPORTS;
		JB_ReportCount++;
		if (JB_ReportCount > MAX_REPORTS)
		    JB_ReportCount = MAX_REPORTS;
		    
		format (string, sizeof (string), "JunkBuster:{FFFF00} Jugador sospechoso con %s: %s (%d, %s), %s.", report, JB_ReturnPlayerName(playerid), playerid, JB_ReturnPlayerIp(playerid), details);
		ForEachPlayer (i)
		...
		return JB_Log (string [20]);
	}
	return 0;
}

PRIVATE: JB_IsPlayerAdmin (playerid)
...
PUBLIC: GlobalUpdate ()
{
    ...
    ForEachPlayer (i)
    {
        if (JB_Variables [MAX_TOTAL_WARNINGS])
        {
            totalwarnings = JB_Warnings [i]{REPORT_MONEY_HACK};
			totalwarnings += JB_Warnings [i]{PAY_FOR_GUNS};
			totalwarnings += JB_Warnings [i]{TELEPORT_HACK};
			totalwarnings += JB_Warnings [i]{AIRBRAKE};
			totalwarnings += JB_Warnings [i]{MAX_SPEED};
			totalwarnings += JB_Warnings [i]{WALLRIDE};
			totalwarnings += JB_Warnings [i]{NO_RELOAD_SAWNOFF};
			totalwarnings += JB_Warnings [i]{SS_HEALTH};
			totalwarnings += JB_Warnings [i]{ACTIVE_GMC};
			totalwarnings += JB_Warnings [i]{SPAWN_VEHICLES};
			totalwarnings += JB_Warnings [i]{VEHICLE_REPAIR};
			totalwarnings += JB_Warnings [i]{NO_RELOAD};
			totalwarnings += JB_Warnings [i]{DRIVE_BY};
			totalwarnings += JB_Warnings [i]{ARMED_VEHICLES};
			totalwarnings += JB_Warnings [i]{SPAWNKILL};
			totalwarnings += JB_Warnings [i]{CAR_JACK_HACK};
			totalwarnings += JB_Warnings [i]{VEHICLE_TELEPORT};
			totalwarnings += JB_Warnings [i]{CBUG};
			totalwarnings += JB_Warnings [i]{JOYPAD};
			totalwarnings += JB_Warnings [i]{CHECKPOINT_TELEPORT};
				
			if (totalwarnings >= JB_Variables [MAX_TOTAL_WARNINGS])
			{
			    switch (JB_Variables [TOO_MANY_WARNS_ACTION])
			    {
			        case 1:
					{
					    JB_Kick (i, "Sospechoso de ser un jugador con cheats");
					    continue;
					}
					...
					case 3:
					{
					    TempBan (i, 1, "Sospechoso de ser un jugador con cheats");
					    continue;
					}
			    }
			}
        ...
        if (DOB::GetBit (JB_FullyConnected, i))
        {
	        nokeypressed = (GetTickCount () - JB_PlayerInfo [i][JB_pLastKeyPressed]) >= (60000 * JB_Variables [AFK]);
	        if ((!JB_IsPlayerAdmin (i) || !JB_Variables [ADMIN_IMMUNITY]) && JB_Variables [AFK]
				&& (nokeypressed || IsPlayerInRangeOfPoint (i, 3.0, JB_PlayerInfo [i][JB_pAFKPos][0], JB_PlayerInfo [i][JB_pAFKPos][1], JB_PlayerInfo [i][JB_pAFKPos][2])))
	        {
	            JB_Warnings [i]{AFK}++;
	            if (JB_Warnings [i]{AFK} >= JB_Variables [AFK] || nokeypressed)
	            {
	                format (reason, sizeof (reason), "Tiempo de inactividad demasiado largo [Maximo %d min(s)]", JB_Variables [AFK]);
	                JB_Kick (i, reason);
	            }
	        }
			else
			{
			    JB_Warnings [i]{AFK} = 0;
				if (IsPlayerInValidState (i) && JB_Variables [ACTIVE_GMC] && !IsPlayerNPC (i) && JB_PlayerInfo [i][JB_pKillingSpree] >= 5 && !JB_IsPlayerAdmin (i) && HasTimePassed (JB_PlayerInfo [i][JB_pLastGMC], 60000 * 5))
					playerlist [players++] = i;
			}
		}
        GetPlayerPos (i, JB_PlayerInfo [i][JB_pAFKPos][0], JB_PlayerInfo [i][JB_pAFKPos][1], JB_PlayerInfo [i][JB_pAFKPos][2]);
	}
	
	if (players) // 0 when JB_Variables [ACTIVE_GMC] = 0
		JB_GodModeCheck (playerlist [random (players)]);
	return 1;
}
...
PUBLIC: AirbrakeCheck ()
{
	if (JB_Variables [AIRBRAKE])
	{
	    ...
	    ForEachPlayer (i)
	    {
	        if (!JB_SyncInfo [i][SYNC_TYPE_POS][JB_sSyncTime] && DOB::GetBit (JB_FullyConnected, i))
	        {
		        if (GetPlayerSurfingVehicleID (i) == INVALID_VEHICLE_ID && GetPlayerSurfingObjectID (i) == INVALID_OBJECT_ID && JB_GetPlayerSpeed (i, true) < 30 && (JB_PlayerInfo [i][JB_pCurrentState] == PLAYER_STATE_DRIVER || JB_PlayerInfo [i][JB_pCurrentState] == PLAYER_STATE_ONFOOT))
		        {
					speed = floatround ((GetPlayerDistanceFromPoint (i, JB_PlayerInfo [i][JB_pOldAirbrakePos][0], JB_PlayerInfo [i][JB_pOldAirbrakePos][1], JB_PlayerInfo [i][JB_pOldAirbrakePos][2]) / float (t - JB_PlayerInfo [i][JB_pLastAirbrakeCheck])) * 1000.0);
                    GetPlayerPos (i, JB_PlayerInfo [i][JB_pOldAirbrakePos][0], JB_PlayerInfo [i][JB_pOldAirbrakePos][1], JB_PlayerInfo [i][JB_pOldAirbrakePos][2]);
					if (speed > JB_Variables [AIRBRAKE] && JB_PlayerInfo [i][JB_pLastAirbrakeSpeed] > JB_Variables [AIRBRAKE])
					{
					    JB_Warnings [i]{AIRBRAKE}++;
						if (JB_Warnings [i]{AIRBRAKE} < MAX_CHECKS)
						{
							format (reason, sizeof (reason), "altura: %.2f, ~%d m/s, vehiculo: %s", JB_PlayerInfo [i][JB_pOldAirbrakePos][2], speed, JB_GetVehicleName (GetPlayerVehicleID (i)));
                            JB_PlayerInfo [i][JB_pLastAirbrakeSpeed] = 0;
							JunkBusterReport (i, "airbrake", reason);
						}
						else
						{
						    if (JB_AirbrakeDetection == 1)
								JB_Kick (i, "Airbrake");
							else
							    JB_Kick (i, "Airbrake");
							continue;
					...
					JB_PlayerInfo [i][JB_pLastAirbrakeSpeed] = speed;
				}
				else
				{
				    GetPlayerPos (i, JB_PlayerInfo [i][JB_pOldAirbrakePos][0], JB_PlayerInfo [i][JB_pOldAirbrakePos][1], JB_PlayerInfo [i][JB_pOldAirbrakePos][2]);
			...
			JB_PlayerInfo [i][JB_pLastAirbrakeCheck] = t;
	    }
	    return 1;
	}
	return 0;
...
PUBLIC: QuickTurnCheck ()
{
	if (JB_Variables [QUICK_TURN])
	{
		...
		ForEachPlayer (i)
		{
			if (DOB::GetBit (JB_FullyConnected, i) && JB_PlayerInfo [i][JB_pCurrentState] == PLAYER_STATE_DRIVER && !JB_IsPlayerAdmin (i))
			{
				...
				if (speed > 15 && abs(JB_PlayerInfo [i][JB_pOldSpeed] - speed) < 25.0 && (170.0 < ad < 190.0))
					if ((x < 0.0) != (JB_PlayerInfo [i][JB_pVelocity][0] < 0.0) && (y < 0.0) != (JB_PlayerInfo [i][JB_pVelocity][1] < 0.0) && (z < 0.0) != (JB_PlayerInfo [i][JB_pVelocity][2] < 0.0))
						JB_Kick (i, "Quick turn"); //He must have used quick turn! I think there is no other way to satisfy the statements above...
...
PUBLIC: JunkBuster ()
{
	...
	ForEachPlayer (i)
	{
		if (DOB::GetBit (JB_FullyConnected, i) && GetPlayerPos (i, x, y, z))
		{
		    vehicleid = GetPlayerVehicleID (i);
			if (vehicleid != INVALID_VEHICLE_ID)
				GetVehiclePos (vehicleid, JB_VehiclePos [vehicleid][0], JB_VehiclePos [vehicleid][1], JB_VehiclePos [vehicleid][2]);
			...
			if (JB_Variables [MONEY_HACK])
			{
				...
				if (var > JB_PlayerInfo [i][JB_pMoney])
				{
				    if (JB_Variables [REPORT_MONEY_HACK] && !JB_IsPlayerAdmin (i))
				    {
                        if (!HasTimePassed (JB_PlayerInfo [i][JB_pLastUpdate], 2000))
                        {
                            JB_Warnings [i]{MONEY_HACK}++;
                            if (JB_Warnings [i]{MONEY_HACK} >= MAX_CHECKS)
                            {
                                JB_Warnings [i]{MONEY_HACK} = 0;
								JB_Warnings [i]{REPORT_MONEY_HACK}++;
								if (JB_Warnings [i]{REPORT_MONEY_HACK} >= MAX_CHECKS && JB_Variables [REPORT_MONEY_HACK] > 1)
								{
								    JB_Kick (i, "Moneyhack");
								    continue;
								}
								...
								{
	                                format (reason, sizeof (reason), "$%d expected but $%d found", JB_PlayerInfo [i][JB_pMoney], var);
									JunkBusterReport (i, "moneyhack", reason);
									// Resetting seems to be a good idea.
									ResetPlayerMoney (i);
									GivePlayerMoney (i, JB_PlayerInfo [i][JB_pMoney]);
								}
                            }
                        }
                        else
			...
			if (JB_PlayerInfo [i][JB_pAirbraking] >= MAX_CHECKS)
				JB_PlayerInfo [i][JB_pAirbraking] = 0;

            if (JB_PlayerInfo [i][JB_pWallriding] >= MAX_CHECKS)
				JB_PlayerInfo [i][JB_pWallriding] = 0;

			if (!JBGMC::Progress {i})
			{
			    freezed = bool: DOB::GetBit (JB_Freezed, i);
				if (freezed && JB_Variables [FREEZE_UPDATE])//Prevent that cheaters unfreeze themselves.
				{
					TogglePlayerControllable (i, false);
					SetSyncTime (i, SYNC_TYPE_VEHICLE);
				}

	            if (JB_PlayerInfo [i][JB_pFreezeTime])
	            {
	                JB_PlayerInfo [i][JB_pFreezeTime]--;
	                if (!JB_PlayerInfo [i][JB_pFreezeTime] && !freezed)
	                    TogglePlayerControllable (i, true);
	            }
            }
            else if (JB_PlayerInfo [i][JB_pFreezeTime])
                JB_PlayerInfo [i][JB_pFreezeTime]--;

			if (JB_PlayerInfo [i][JB_pCurrentState] == PLAYER_STATE_ONFOOT)
			{
				if (IsPlayerBuyingInAmmuNation (i))
				{
				    DOB::SetBit (JB_BuyingInAmmuNation, i, true);
				...
				{
				    DOB::SetBit (JB_BuyingInAmmuNation, i, false);
					JB_PlayerInfo [i][JB_pVendingMachineUsed] = 5;
					SetSyncTime (i, SYNC_TYPE_HEALTH);
				}
			}

			if (!JB_IsPlayerAdmin (i) && IsPlayerInValidState (i) && !DOB::GetBit (JB_AntiBugKilled, i)) //!JB_PlayerInfo [i][JB_pAntiBugKilled])
			{
			    if (JB_Variables [CHECK_WALK_ANIMS] && GetPlayerWeapon (i) != 46 && JB_PlayerInfo [i][JB_pCurrentState] == PLAYER_STATE_ONFOOT && !JB_PlayerPedAnims && GetPlayerSkin (i) != 0 && GetPlayerAnimationIndex (i) == 1231)
			    {
			        JB_Kick (i, "Cheats detectados");
			        continue;
			    }
			    
			    if (JB_Variables [JOYPAD] && !JB_PlayerInfo [i][JB_pFreezeTime])
				{
				    GetPlayerKeys (i, keys, updown, leftright);
				    if((updown != 128 && updown != 0 && updown != -128) || (leftright != 128 && leftright != 0 && leftright != -128))
				    {
				        if (++JB_Warnings [i]{JOYPAD} >= JB_Variables [JOYPAD])
				        	JB_Kick (i, "Usar gamepad");
						else
						{
						    JB_PlayerInfo [i][JB_pFreezeTime] = 5; // Freeze for 5 seconds.
						    SetSyncTime (i, SYNC_TYPE_VEHICLE);
							TogglePlayerControllable (i, false);
							SendClientMessage (i, JB_RED, ""COL_ADM"Administración:"COL_WHITE" Por favor, deje de usar un joypad/gamepad. Usa el teclado y el ratón.");
						}
				    }
				}

			    if (DOB::GetBit (JB_GunBought, i) && DOB::GetBit (JB_BuyingInAmmuNation, i) && JB_PlayerInfo [i][JB_pLastBoughtWeapon] != 0)
			    {
			        if (++JB_PlayerInfo [i][JB_pBuyingGuns] >= 2)
			        {
				        if (GetTickCount () - JB_PlayerInfo [i][JB_pLastMoneyChange] > 5000)
				        {
				            if (++JB_Warnings [i]{PAY_FOR_GUNS} >= JB_Variables [PAY_FOR_GUNS])
				            {
				                reason [0] = '\0';
				                GetWeaponName (JB_PlayerInfo [i][JB_pLastBoughtWeapon], reason, sizeof (reason));
				                format (reason, sizeof (reason), "No pagan por un arma: %s", reason);
					            JB_Kick (i, reason);
					            continue;
				            }
				        }
				        else if (JB_PlayerInfo [i][JB_pLastLostMoney] < AmmuNationInfo [JB_PlayerInfo [i][JB_pLastBoughtWeapon]][0])
				        {
				            if (++JB_Warnings [i]{PAY_FOR_GUNS} >= JB_Variables [PAY_FOR_GUNS])
				            {
					            reason [0] = '\0';
	                    ...
	                    DOB::SetBit (JB_GunBought, i, false);
	                    JB_PlayerInfo [i][JB_pBuyingGuns] = 0;
                    }
			    }
			    else
			        JB_PlayerInfo [i][JB_pBuyingGuns] = 0;
				...
				{
					var = JB_GetPlayerFPS (i);
					if (var < JB_Variables [MIN_FPS])
					{
						JB_Warnings [i]{MIN_FPS}++;
						if (JB_Warnings [i]{MIN_FPS} >= (MAX_CHECKS * 30))//Constantly low FPS = kick
						{
							format (reason, sizeof (reason), "Bajos FPS (%d, min %d)", var, JB_Variables [MIN_FPS]);
							JB_Kick (i, reason);
							continue;
						}
						else if (JB_Warnings [i]{MIN_FPS} % (MAX_CHECKS * 10) == 0)
						{
							JB_SendFormattedMessage (i, JB_RED, ""COL_ADM"Administración:"COL_WHITE" Por favor, arregla tu Framerate (FPS) o te expulsaran! (Mínimo %d, tu FPS: %d)", JB_Variables [MIN_FPS], var);
							SendClientMessage (i, JB_RED, ""COL_ADM"Administración:"COL_WHITE" Presionar F7 una vez puede ayudar a arreglarlo. (Elimina los contornos de las letras en la ventana de chat.)");
						}
					}
				}

				if (!JB_SyncInfo [i][SYNC_TYPE_POS][JB_sSyncTime])
				{
					...
					{
						if (JB_PlayerInfo [i][JB_pCurrentState] != PLAYER_STATE_PASSENGER && !surfing && !IsPlayerInRangeOfPoint (i, 350.0, JB_PlayerInfo [i][JB_pCurrentPos][0], JB_PlayerInfo [i][JB_pCurrentPos][1], JB_PlayerInfo [i][JB_pCurrentPos][2]))
						{
							if (!IsPlayerInRangeOfPoint (i, 5.0, JB_PlayerInfo [i][JB_pSetPos][0], JB_PlayerInfo [i][JB_pSetPos][1], JB_PlayerInfo [i][JB_pSetPos][2]))
							{
								JB_GetPlayer2DZone (i, reason, sizeof (reason));
								JB_Warnings [i]{TELEPORT_HACK}++;
								if (JB_Variables [TELEPORT_HACK])
								{
									if (IsCheatPosition (i))
									{
										format (reason, sizeof (reason), "Cheats de teletransportarse [Codigo 1]: %s", reason);
										JB_Kick (i, reason);
										continue;
									}
									else if (JB_Variables [PICKUP_TELEPORT] && IsPickupPosition (i)) // IsPickupPosition will only be executed when Anti-PickupTeleport is enaled.
									{
									    format (reason, sizeof (reason), "Cheats de teletransportarse [Codigo 3]: %s", reason);
										JB_Kick (i, reason);
										continue;
									}
									else if (JB_Warnings [i]{TELEPORT_HACK} >= MAX_CHECKS)
									{
										format (reason, sizeof (reason), "Cheats de teletransportarse [Codigo 2]: %s", reason);
										JB_Kick (i, reason);
										continue;
									}
									else
					...
					// This will only be executed when JB_AirbrakeDetection == 0 (slow mode)
					if (!JB_AirbrakeDetection && JB_Variables [AIRBRAKE] && !surfing && (JB_PlayerInfo [i][JB_pCurrentState] == PLAYER_STATE_ONFOOT || (JB_PlayerInfo [i][JB_pCurrentState] == PLAYER_STATE_DRIVER && JB_GetPlayerSpeed (i, true) < 30)))
					{
					    var = floatround (GetPlayerDistanceFromPoint (i, JB_PlayerInfo [i][JB_pCurrentPos][0], JB_PlayerInfo [i][JB_pCurrentPos][1], JB_PlayerInfo [i][JB_pCurrentPos][2]));
						if (var >= JB_Variables [AIRBRAKE])
						{
							JB_PlayerInfo [i][JB_pAirbraking]++;
							if (JB_PlayerInfo [i][JB_pAirbraking] >= (MAX_CHECKS - 1))
							{
								JB_Warnings [i]{AIRBRAKE}++;
								if (JB_Warnings [i]{AIRBRAKE} < MAX_CHECKS)
								{
									format (reason, sizeof (reason), "altura: %.2f, ~%d m/s, vehiculo: %s", z, var, JB_GetVehicleName (vehicleid));
									JunkBusterReport (i, "airbrake", reason);
								}
								else
								{
				...
				if (JB_Variables [FLY_HACK] && ivar [3] > JB_Variables [FLY_HACK] && z > 12.0 && (var == 1543 || var == 1538 || var == 1539))
				{
				    JB_Kick (i, "Fly hack"); // Obvious noob is obvious.
				    continue;
				}

				if (JB_PlayerInfo [i][JB_pCurrentState] == PLAYER_STATE_DRIVER)
				{
				    if (JB_Variables [TANK_MODE])
					{
					...
					if (JB_Variables [TUNING_HACK])
					{
					    for (new j = 0; j < MAX_COMPONENT_SLOTS; ++j)
					    {
					        componentid = GetVehicleComponentInSlot (vehicleid, j);
					        if (componentid != 0 && componentid != (JB_VehicleComponents [vehicleid]{j} + 999))
					        {
					            JB_Kick (i, "Tuning hack");
					            SetVehicleToRespawn (vehicleid);
					            continue;
					        }
					    }
					...
					if (JB_Variables [MAX_SPEED])
					{
      					ivar [0] = JB_GetVehicleMaxSpeed (vehicleid);
						if (!IsPlayerInPlane (i))
						{
						    if (JB_Variables [SPEEDHACK_DETECTION] == 0)
						    {
						        var = JB_GetPlayerSpeed (i, JB_Variables [SPEED_3D]);
								{
									/*if (var > JB_Variables [MAX_SPEED])
									{
										JB_Warnings [i]{MAX_SPEED}++;
										SetSyncTime (i, SYNC_TYPE_VEHICLE);
										if (JB_Warnings [i]{MAX_SPEED} < MAX_CHECKS)
										{
											format (reason, sizeof (reason), "%d KM/H con %s (Max %d)", var, JB_GetVehicleName (vehicleid), ivar [0]);
											JunkBusterReport (i, "speedhack [Codigo 1]", reason);
											continue;
										}
									}*/
									if (JB_Variables [SPEEDHACK_ADVANCED] && var > (ivar [0] + JB_Variables [SPEEDHACK_ADVANCED]))
									{
									    JB_Warnings [i]{MAX_SPEED}++;
										SetSyncTime (i, SYNC_TYPE_VEHICLE);
										if (JB_Warnings [i]{MAX_SPEED} < MAX_CHECKS)
										{
											format (reason, sizeof (reason), "%d KM/H con %s (Max %d)", var, JB_GetVehicleName (vehicleid), ivar [0]);
											JunkBusterReport (i, "speedhack [Codigo 2]", reason);
											JB_Kick (i, "Speedhack [Codigo 2]");
											continue;
										}
									}
								}
							...
							if (JB_Variables [WALLRIDE])
							{
							    ...
							    if (75.0 < abs (rz) < 105.0)
							    {
									...
									if (floatround (rz * 160.0) < JB_Variables [WALLRIDE] && var > 10)
									{
									    JB_PlayerInfo [i][JB_pWallriding]++;
									    if (JB_PlayerInfo [i][JB_pWallriding] == MAX_CHECKS)
									    {
										    JB_Warnings [i]{WALLRIDE}++;
										    if (JB_Warnings [i]{WALLRIDE} >= MAX_CHECKS)
										    {
										        JB_Kick (i, "Wallride");
										        continue;
										    }
										    ...
										    {
										        format (reason, sizeof (reason), "con %s", JB_GetVehicleName (vehicleid));
				...
				if (IsWeaponForbiddenForPlayer (i, var))
				{
					...
					format (reason, sizeof (reason), "Usar cheats de armas (%s)", reason);
					JB_Kick (i, reason);
					continue;
				}

				if (JB_Variables [WEAPON_HACK] && !JB_SyncInfo [i][SYNC_TYPE_WEAPONS][JB_sSyncTime])
				{
				...
				{
					if (var == 26 && JB_PlayerInfo [i][JB_pCurrentState] == PLAYER_STATE_ONFOOT)
					{
						var = GetPlayerAmmo (i);
						ivar [0] = (JB_PlayerInfo [i][JB_pSawnOffAmmo] - var);
						if (ivar [0] > 5 && ivar [0] < 11)//6 - 10 shots per second with sawn-off is cheating!
						{
							...
							{
								JB_Kick (i, "No recargar (Sawn-off Shotgun) [Codigo 2]");
								continue;
							}
						}
				...
				if (JB_Variables [HEALTH_HACK])
				{
					...
					if (health > 100.0)
					{
						JB_Kick (i, "Health hack [Codigo 1]");
						continue;
					}
				}
				...
				if (JB_Variables [ARMOUR_HACK])
				{
					if (armour > 100.0)
					{
						JB_Kick (i, "Armour hack [Codigo 1]");
						continue;
					}
				}

				if (JB_Variables [SS_HEALTH])
				{
					if (health > JB_PlayerInfo [i][JB_pHealth] && !JB_PlayerInfo [i][JB_pVendingMachineUsed] && !JB_SyncInfo [i][SYNC_TYPE_HEALTH][JB_sSyncTime])
					{
						JB_Warnings [i]{SS_HEALTH}++;
						if (JB_Warnings [i]{SS_HEALTH} >= MAX_CHECKS)
							JB_Kick (i, "Health hack [Codigo 2]");
						continue;
					}
					else if (armour > JB_PlayerInfo [i][JB_pArmour] && !JB_SyncInfo [i][SYNC_TYPE_ARMOUR][JB_sSyncTime])
					{
						JB_Warnings [i]{SS_HEALTH}++;
						if (JB_Warnings [i]{SS_HEALTH} >= MAX_CHECKS)
							JB_Kick (i, "Armour hack [Codigo 2]");
						continue;
					}
					else
				...
				if (JB_PlayerInfo [i][JB_pPingCheckProgress] == MAX_PING_CHECKS)
				{
					if (JB_Variables [MAX_PING])
					{
						...
						{
							format (reason, sizeof (reason), "Ping demasiado alto. [%d/%d]", var, JB_Variables [MAX_PING]);
							JB_Kick (i, reason);
							continue;
						}
					}
...
PRIVATE: SplitIp (ipstring [])
{
	...
	{
		new ip = SplitIp (JB_ReturnPlayerIp(playerid));
	...
	PUBLIC: BanIp (ipstring [])
	{
		...
		if (ip != 0xFFFFFFFF && IpBanCount < sizeof (IpBans))
		{
		    ...
		    JB_LogEx ("IP %s ha sido baneada.", ipstring);
			SaveIpBans ();
			ForEachPlayer (j)
				if (IsSameIpEx (ip, SplitIp (JB_PlayerInfo [j][JB_pIp])))
					JB_Kick (j, "IP baneada");
			return 1;
		}
		JB_LogEx ("No se pudo banear la IP %s!", ipstring);
		return 0;
	}
	...
	PUBLIC: UnbanIp (ipstring [])
	{
		...
		for (new i = 0; i < IpBanCount; ++i)
		{
			if (IsSameIp (IpBans [i], ip))
			{
				...
				JB_LogEx ("IP %s ha sido desbaneada", ipstring);
				SaveIpBans ();
				return 1;
			}
		}
		JB_LogEx ("No se pudo desbanear la IP %s!", ipstring);
		return 0;
	}
	...
	PRIVATE: LoadIpBans ()
	{
		if (DOF2::FileExists (IP_BAN_FILE))
		{
			...
			fclose (f);
			JB_LogEx ("%d IP-bans have been loaded.", IpBanCount);
			return 1;
		}
		else
		{
	...
	PRIVATE: LoadBlacklist ()
	{
		if (DOF2::FileExists (BLACKLIST_FILE))
		{
			...
			fclose (f);
			JB_LogEx ("%d blacklist entries have been loaded.", BlacklistCount);
			return 1;
		}
		else
		{
	...
	PUBLIC: AddPlayerToBlacklist (playerid)
	{
		if (AddNameToBlacklist (JB_ReturnPlayerName(playerid)))
			return JB_Kick (playerid, "Nombre no permitido");
		return 0;
	}
	...
	PUBLIC: IsPlayerOnBlacklist (playerid)
	{
		...
		{
		    strunpack (string, Blacklist [i]);
			if (!strcmp (JB_ReturnPlayerName(playerid), string, false))
				return true;
		}
		return false;
	}
	...
	PRIVATE: LoadWhitelist ()
	{
		if (DOF2::FileExists (WHITELIST_FILE))
		{
			...
			fclose (f);
			JB_LogEx ("%d whitelist entries have been loaded.", WhitelistCount);
			return 1;
		}
		else
		{
	...
	PUBLIC: TempBanCheck (playerid)
	{
		...
		{
			if (!strcmp (TempBanInfo [i][tbName], JB_ReturnPlayerName(playerid), false))
			{
				if (TempBanInfo [i][tbTime] > time)
				{
					...
					TempBanInfo [i][tbIp] = JB_ReturnPlayerIp(playerid);
					JB_SendFormattedMessage (playerid, JB_RED, ""COL_ADM"Administración:"COL_WHITE" Usted esta baneado temporalmente por %d día(s), %d hora(s), %d minuto(s) y %d segundo(s)!", days, hours, minutes, seconds);
					JB_LogEx ("%s (%s) ha sido baneado por evadir el baneo.", JB_ReturnPlayerName(playerid), JB_ReturnPlayerIp(playerid));
					BanEx (playerid, "Evasion de baneo");
					return 1;
				}
				else
	...
	PUBLIC: TempBan (playerid, days, reason [])
	{
		if (days > 0)
		{
		    if (TempBanCount < sizeof (TempBanInfo))
		    {
				...
				strcpy (TempBanInfo [TempBanCount][tbIp], JB_ReturnPlayerIp(playerid), 16);
				strcpy (TempBanInfo [TempBanCount][tbName], JB_ReturnPlayerName(playerid), MAX_PLAYER_NAME);
				++TempBanCount;
				format (string, sizeof (string), "%s [%d día(s)]", reason, days);
				JB_Kick (playerid, string);
				SaveTempBans ();
				return 1;
		    }
		}
	...
	PUBLIC: DeleteTempBan (name [])
	{
		for (new i = 0; i < TempBanCount; ++i)
		{
			if (!strcmp (TempBanInfo [i][tbName], name, false))
			{
				...
				JB_LogEx ("JUgador '%s' (%s) ha sido desbaneado.", name, TempBanInfo [i][tbIp]);
	...
	PRIVATE: LoadTempBans ()
	{
		if (DOF2::FileExists (TEMP_BAN_FILE))
		{
			...
			while (fread (f, string, sizeof (string)) && TempBanCount < sizeof (TempBanInfo))
			{
				JB_StripNewLine (string);
				if (!isnull (string))
					if (!JB_sscanf (string, "iss", TempBanInfo [TempBanCount][tbTime], TempBanInfo [TempBanCount][tbIp], TempBanInfo [TempBanCount][tbName]))
						TempBanCount++;
			}
			fclose (f);
			JB_LogEx ("%d temporary bans have been loaded.", TempBanCount);
			return 1;
		}
		else
...
PRIVATE: ConfigJunkBuster ()
{
	...
	{
	    JB_Homepage = "the homepage of this server";
	    DOF2::SetString (CONFIG_FILE, "Homepage", JB_Homepage, "Misc");
	}
	else
	    strcpy (JB_Homepage, DOF2::GetString (CONFIG_FILE, "Homepage", "Misc"));

	for (new i = 0; i < MAX_JB_VARIABLES; ++i)
	{
		if (DOF2::IsSet (CONFIG_FILE, JB_VariableNames [i]))
			JB_Variables [i] = DOF2::GetInt (CONFIG_FILE, JB_VariableNames [i]);
		else
			DOF2::SetInt (CONFIG_FILE, JB_VariableNames [i], JB_Variables [i]);
	}
...
PRIVATE: JB_GetDate ()
{
	...
	format (date, sizeof (date), "%d. %s %d", day, JB_GetMonth (month), year);
	return date;
}

PRIVATE: JB_GetMonth (month)
{
	new string [16] = "Mes desconocido";
...
PRIVATE: JB_StripNewLine (string [])
{
...
PUBLIC: JB_GetPlayerFPS (playerid)
{
	new fps;
...
PUBLIC: JB_GetPlayerSpeed (playerid, get3d)
{
...
PRIVATE: JB_GetVehicleName (vehicleid)
{
	new
		name [32] = "Unknown",
		modelid = GetVehicleModel (vehicleid) - 400;
		
	if (modelid >= 0 && modelid < sizeof (JB_VehicleInfo))
	    strunpack (name, JB_VehicleInfo [modelid][JB_vName]);
	return name;
}

PRIVATE: JB_GetVehicleMaxSpeed (vehicleid)
{
	new modelid = GetVehicleModel (vehicleid) - 400;
		
	if (modelid >= 0 && modelid < sizeof (JB_VehicleInfo))
	    return JB_VehicleInfo [modelid][JB_vMaxSpeed];
	return 9999;
}

PRIVATE: JB_GetSquaredDistance (Float: x1, Float: y1, Float: z1, Float: x2, Float: y2, Float: z2)
{
	x1 -= x2;
	y1 -= y2;
	z1 -= z2;
...
PRIVATE: JB_GetPlayer2DZone (playerid, zone [], len = sizeof (zone)) //Credits to Cueball, Betamaster, Mabako, and Simon (for finetuning).
{
	new
		Float: x,
		Float: y,
	...
	GetPlayerPos (playerid, x, y, z);
 	for (new i = 0; i != sizeof (JB_SAZones); ++i)
 	{
		if (x >= JB_SAZones [i][JB_SAZONE_AREA][0] && x <= JB_SAZones [i][JB_SAZONE_AREA][3] && y >= JB_SAZones [i][JB_SAZONE_AREA][1] && y <= JB_SAZones [i][JB_SAZONE_AREA][4])
		{
		    strunpack (zone, JB_SAZones [i][JB_SAZONE_NAME], len);
			return i;
		}
	}
	return -1;
...
PRIVATE: ShowPlayerHackCodigoDialog (playerid)
{
	new string [1024];
	
	strcat (string, "{0CB7EB}No recargar:{FFFFFF}\n\tCodigo 1: El jugador probablemente usa 2 tiros con escopeta cortada.\n\tCodigo 2: El jugador está disparando muy rápido con una escopeta recortada. Hack, no 2 tiros.\n\n");
	strcat (string, "{0CB7EB}Teleporthack:{FFFFFF}\n\tCodigo 1: El jugador se teletransporta a una posición conocida por las herramientas de trampas.\n\tCodigo 2: Teletransporte sospechoso, posición no definida. ¿Se teletransportó a otro jugador?\n\tCodigo 3: Teletransportarse a un pickup.\n\n");
	strcat (string, "{0CB7EB}Speedhack:{FFFFFF}\n\tCodigo 2: La velocidad del jugador es mayor que el límite del modelo de vehículo.\n\n");
	strcat (string, "{0CB7EB}Health-/armourhack:{FFFFFF}\n\tCodigo 1: La salud/armadura es superior a 100.\n\tCodigo 2: La salud/armadura del jugador es más alta de lo que JunkBuster realmente espera.");
	ShowPlayerDialog (playerid, DIALOG_HACKCodigoS, DIALOG_STYLE_MSGBOX, "Tipos de códigos JunkBuster", string, "Cerrar", "");
}
...
PRIVATE: ShowPlayerSetvarDialog (playerid, var)
{
	...
	strunpack (description, JB_VarDescription [var]);
...
{
	new
		string [(sizeof (JB_Reports []) * 4 + 8) * MAX_REPORTS],
		buf [sizeof (JB_Reports []) * 4 + 8];
		
	for (new i = 0; i < JB_ReportCount; ++i)
	{
		buf [0] = '\0';
	    strunpack (buf, JB_Reports [(JB_ReportIndex - 1 - i) % JB_ReportCount]);
	    if (buf [0])
	    {
			strcat (string, buf);
			strcat (string, "\n");
	...
	if (string [0])
		ShowPlayerDialog (playerid, DIALOG_REPORTS, DIALOG_STYLE_LIST, "Reportes de JunkBuster", string, "Cerrar", "");
	else
	    ShowPlayerDialog (playerid, DIALOG_REPORTS, DIALOG_STYLE_MSGBOX, "Reportes de JunkBuster", "No hay reportes.", "Cerrar", "");
}

//==============================================================================

PUBLIC: JB_GodModeCheck (playerid)
{
	if (IsPlayerConnected (playerid) && !IsPlayerNPC (playerid) && IsPlayerInValidState (playerid))
	{
		if (!HasTimePassed (JB_PlayerInfo [playerid][JB_pLastUpdate], 1000) &&  !DOB::GetBit (JB_Freezed, playerid))
		{
			new
				Float: x,
				Float: y,
			...
			TogglePlayerControllable (playerid, false);
			JB_SetPlayerPos (playerid, JBGMC::OldPos [playerid][0], JBGMC::OldPos [playerid][1], JBGMC::OldPos [playerid][2] + 2000.0);
...
PRIVATE: JBGMC::EndCheck (playerid, bool: reset = true)
{
    if (IsPlayerConnected (playerid) && JBGMC::Progress {playerid} != 0)
	{
	    ...
	    {
		    SetCameraBehindPlayer (playerid);
		    JB_SetPlayerHealth (playerid, JBGMC::OldHealth [playerid]);
		    JB_SetPlayerArmour (playerid, JBGMC::OldArmour [playerid]);
		    JB_SetPlayerPos (playerid, JBGMC::OldPos [playerid][0], JBGMC::OldPos [playerid][1], JBGMC::OldPos [playerid][2]);
		    if (JBGMC::VehicleID [playerid] != INVALID_VEHICLE_ID)
		    	JB_PutPlayerInVehicle (playerid, JBGMC::VehicleID [playerid], JBGMC::Seat {playerid});
	    }
	...
	if (JBGMC::Progress {playerid} != 0 && IsPlayerInValidState (playerid))
	{
	    switch (JBGMC::Progress {playerid})
	    {
	        case 1:
	        {
				...
				JB_SetPlayerHealth (playerid, JBGMC::NewHealth [playerid]);
				JB_SetPlayerArmour (playerid, 0.0);
...
{
	JB_PlayerPedAnims = true;
	return 1;
}

CALLBACK: OnSetPlayerName (playerid, name [])
{
	strcpy (JB_ReturnPlayerName(playerid), name, MAX_PLAYER_NAME);
	return 1;
}
...
CALLBACK: OnPlayerMoneyChange (playerid, oldmoney, newmoney)
{
	...
	{
	    JB_PlayerInfo [playerid][JB_pLastLostMoney] = oldmoney - newmoney;
	    JB_PlayerInfo [playerid][JB_pLastMoneyChange] = GetTickCount ();
	}
	return 1;
}
...
CALLBACK: OnPlayerBuyGun (playerid, weaponid, ammo)
{
	...
	{
	    JB_PlayerInfo [playerid][JB_pLastBoughtWeapon][0] = weaponid;
		JB_PlayerInfo [playerid][JB_pLastBoughtWeapon][1] = ammo;
	}
	return 1;
}
...
CALLBACK: OnPlayerGodMode (playerid)
{
	...
	JunkBusterReport (playerid, "godmode", "sin detalles");
	return 1;
}
...
CALLBACK: OnPlayerReport (playerid, reporterid, report [])
{
	...
	JB_LogEx ("%s (%s) has reported %s (%s). Reason: %s", JB_PlayerInfo [reporterid][JB_pName], JB_PlayerInfo [reporterid][JB_pIp], JB_ReturnPlayerName(playerid), JB_ReturnPlayerIp(playerid), report);
	if (JB_Variables [ACTIVE_GMC] && !JB_IsPlayerAdmin (playerid) && HasTimePassed (JB_PlayerInfo [playerid][JB_pLastGMC], 60000 * 5) && (strfind (report, "god", true) != -1 || strfind (report, "health", true) != -1))
	{
...
{
	if (JB_Variables [SPAWN_VEHICLES] && !passenger_seat && JB_PlayerInfo [playerid][JB_pCurrentState] == PLAYER_STATE_ONFOOT
		&& GetVehicleDistanceFromPoint (vehicleid, JB_VehiclePos [vehicleid][0], JB_VehiclePos [vehicleid][1], JB_VehiclePos [vehicleid][2]) > 50.0)
	{
	    new Float: x, Float: y, Float: z;
 		...
 		{
 		    if (++JB_Warnings [playerid]{SPAWN_VEHICLES} >= JB_Variables [SPAWN_VEHICLES])
 		    {
 		        new reason [64] = "Spawning vehicle: ";
 		        
 		        strcat (reason, JB_GetVehicleName (vehicleid));
 		        JB_Kick (playerid, reason);
 		        return 0;
 		    }
 		    else
 		        JunkBusterReport (playerid, "spawning vehicle", JB_GetVehicleName (vehicleid));
 		}
	}
...
CALLBACK: OnVehicleMod(playerid, vehicleid, componentid)
{
	if (1 <= GetPlayerInterior (playerid) <= 3)
		JB_VehicleComponents [vehicleid]{GetVehicleComponentType (componentid)} = componentid - 999;
	return 1;
}

CALLBACK: OnVehiclePaintjob(playerid, vehicleid, paintjobid)
{
	JB_PlayerInfo [playerid][JB_pVehicleHealth] = 1000.0;
	SetSyncTime (playerid, SYNC_TYPE_VEHICLE);
	return 1;
}
...
CALLBACK: OnPlayerSpawn (playerid)
{
	...
	for (i = 0; i < 3; ++i)
		WeaponUpdate (playerid, JB_SpawnWeapons [playerid][i][0], JB_SpawnWeapons [playerid][i][1]);
		
	DOB::SetBit (JB_FullyConnected, playerid, true);
	DOB::SetBit (JB_Freezed, playerid, false);
    DOB::SetBit (JB_Spectating, playerid, false);
	...
	JB_PlayerInfo [playerid][JB_pAFKPos][0] = JB_PlayerInfo [playerid][JB_pAFKPos][1] = JB_PlayerInfo [playerid][JB_pAFKPos][2] = 99999.9;
	
	if (DOB::GetBit (JB_AntiBugKilled, playerid))
	{
		new
			varname1 [32],
			varname2 [32],
		...
		for (i = 0; i < MAX_WEAPON_SLOTS; ++i)
		{
			...
			if (!DOB::GetBit (JB_PlayerInfo [playerid][JB_pWeaponForbidden], weaponid))
		...
		DOB::SetBit (JB_AntiBugKilled, playerid, false);
		return 1;
	}
...
CALLBACK: OnPlayerUpdate (playerid)
{
	...
	{
	    if (JB_SyncInfo [playerid][i][JB_sSyncTime])
	    {
	        if (JB_PlayerInfo [playerid][JB_pLastUpdate] - JB_SyncInfo [playerid][i][JB_sLastSyncUpdate] > 1000)
	        {
	            JB_SyncInfo [playerid][i][JB_sSyncTime]--;
	            JB_SyncInfo [playerid][i][JB_sLastSyncUpdate] = JB_PlayerInfo [playerid][JB_pLastUpdate];
	        }
	    }
	}

	GetPlayerHealth (playerid, fvar);
	if (fvar < JB_PlayerInfo [playerid][JB_pHealth] || JB_SyncInfo [playerid][SYNC_TYPE_HEALTH][JB_sSyncTime])
		JB_PlayerInfo [playerid][JB_pHealth] = fvar;

	GetPlayerArmour (playerid, fvar);
	if (fvar < JB_PlayerInfo [playerid][JB_pArmour] || JB_SyncInfo [playerid][SYNC_TYPE_ARMOUR][JB_sSyncTime])
	...
	{
		if (JB_PlayerInfo [playerid][JB_pLastDrunkLevel] != ivar)
		{
			new fps = JB_PlayerInfo [playerid][JB_pLastDrunkLevel] - ivar;

			if (fps > 0 && fps < 200)
				JB_PlayerInfo [playerid][JB_pFPS][JB_PlayerInfo [playerid][JB_pFPSIndex]] = fps;
			JB_PlayerInfo [playerid][JB_pLastDrunkLevel] = ivar;
		}
	...
	if (ivar == JB_PlayerInfo [playerid][JB_pCurrentState] && ivar == PLAYER_STATE_DRIVER)
	{
	    GetVehicleHealth (vehicleid, fvar);
	    
	    if (JB_PlayerInfo [playerid][JB_pLastVehicle] != 0 && vehicleid != 0 && !JB_IsPlayerAdmin (playerid))
	    {
	        if (JB_PlayerInfo [playerid][JB_pLastVehicle] != vehicleid)
	        {
			    if (!JB_SyncInfo [playerid][SYNC_TYPE_POS][JB_sSyncTime])
			    {
					if (JB_Variables [VEHICLE_TELEPORT])
					{
						JB_Kick (playerid, "Vehicle teleport");
				        return 0;
			        }
			    }
		    }
		    else if (JB_Variables [VEHICLE_REPAIR] && !JB_SyncInfo [playerid][SYNC_TYPE_VEHICLE][JB_sSyncTime])
		    {
		        if (fvar > JB_PlayerInfo [playerid][JB_pVehicleHealth])
		        {
		            JB_Warnings [playerid]{VEHICLE_REPAIR}++;
		            if (JB_Warnings [playerid]{VEHICLE_REPAIR} >= JB_Variables [VEHICLE_REPAIR])
		            {
                      JunkBusterReport (playerid, "reparar coche con cheats (verificar)", JB_GetVehicleName (vehicleid));
		            }    
		        }
		    }
	    }
	    
	    JB_PlayerInfo [playerid][JB_pVehicleHealth] = fvar;
	    JB_PlayerInfo [playerid][JB_pLastVehicle] = vehicleid;
	}
	else if (ivar != JB_PlayerInfo [playerid][JB_pCurrentState])
	{
	    for (new i = 0; i < MAX_WEAPON_SLOTS; ++i)
	    {
	...
	JB_PlayerInfo [playerid][JB_pCurrentState] = ivar;

	if (ivar == PLAYER_STATE_ONFOOT)
		GetPlayerPos (playerid, JB_PlayerInfo [playerid][JB_pOnFootPos][0], JB_PlayerInfo [playerid][JB_pOnFootPos][1], JB_PlayerInfo [playerid][JB_pOnFootPos][2]);
	...
	if (!JB_IsPlayerAdmin (playerid))
	{
		...
		if (JB_Variables [PAY_FOR_GUNS])
		{
  			new
				weapons,
				slot = GetWeaponSlot (ivar);
            
			if (DOB::GetBit (JB_BuyingInAmmuNation, playerid) && ammo > oldammo [playerid][slot])
			{
				DOB::SetBit (JB_GunBought, playerid, true);
		...
		if (JB_Variables [NO_RELOAD_SAWNOFF] && JB_PlayerInfo [playerid][JB_pOldWeapon] == 26 && ivar == 26)
		{
		...
		if (JB_Variables [SPEEDHACK_DETECTION] != 0 && JB_PlayerInfo [playerid][JB_pCurrentState] == PLAYER_STATE_DRIVER)
		{
	        speed = JB_GetPlayerSpeed (playerid, JB_Variables [SPEED_3D]);
	        maxspeed = JB_GetVehicleMaxSpeed (vehicleid);

			/*if (speed > JB_Variables [MAX_SPEED])
			{
				JB_Warnings [playerid]{MAX_SPEED}++;
				SetSyncTime (playerid, SYNC_TYPE_VEHICLE);
				if (JB_Warnings [playerid]{MAX_SPEED} < MAX_CHECKS)
				{
					format (reason, sizeof (reason), "%d KM/H con %s (Max %d)", speed, JB_GetVehicleName (vehicleid), maxspeed);
					JunkBusterReport (playerid, "speedhack [Codigo 1]", reason);
					return 0;
				}
			}*/
	        if (JB_Variables [SPEEDHACK_ADVANCED] && speed > (maxspeed + JB_Variables [SPEEDHACK_ADVANCED]))
			{
			    JB_Warnings [playerid]{MAX_SPEED}++;
				SetSyncTime (playerid, SYNC_TYPE_VEHICLE);
				if (JB_Warnings [playerid]{MAX_SPEED} < MAX_CHECKS)
				{
					format (reason, sizeof (reason), "%d KM/H con %s (Max %d)", speed, JB_GetVehicleName (vehicleid), maxspeed);
					JunkBusterReport (playerid, "speedhack [Codigo 2]", reason);
					JB_Kick (playerid, "Speedhack [Codigo 2]");
					return 0;
				}
			}
		}
		...
		{
			if ((JB_PlayerInfo [playerid][JB_pLastUpdate] - JB_PlayerInfo [playerid][JB_pLastWeaponUsed][ivar]) > 4000)//Player may just have changed to another weapon without reloading and he doesn't want to abuse bugs.
				JB_PlayerInfo [playerid][JB_pAmmoUsed][ivar] = 0;

			if (JB_PlayerInfo [playerid][JB_pOldWeapon] == ivar)//same weapon.
			{
				if (GetPlayerWeaponState (playerid) == WEAPONSTATE_RELOADING || ammo < 0 || ammo > 9999 || JB_PlayerInfo [playerid][JB_pCurrentState] != PLAYER_STATE_ONFOOT)
					JB_PlayerInfo [playerid][JB_pAmmoUsed][ivar] = 0;
				else
				{
					new ammoused;

					if (!JB_PlayerInfo [playerid][JB_pOldAmmo][ivar])
						JB_PlayerInfo [playerid][JB_pAmmoUsed][ivar] = 0;
					else
					{
						ammoused = (JB_PlayerInfo [playerid][JB_pOldAmmo][ivar] - ammo);
						if (ammoused < 0)
							ammoused = 0;

						JB_PlayerInfo [playerid][JB_pAmmoUsed][ivar] += ammoused;
						if (JB_PlayerInfo [playerid][JB_pAmmoUsed][ivar] < 0)
							JB_PlayerInfo [playerid][JB_pAmmoUsed][ivar] = 0;
					}

					if (JB_PlayerInfo [playerid][JB_pAmmoUsed][ivar] > (AmmoAmount [ivar] * 2) && ammoused > 0)//Player must have switch weapons fast or doesn't reload at all
					{
						JB_Warnings [playerid]{NO_RELOAD}++;
						if ((JB_Variables [NO_RELOAD] == 1 && (JB_Warnings [playerid]{NO_RELOAD} % 10) == 0))
						{
							GetWeaponName (ivar, reason, sizeof (reason));
							JunkBusterReport (playerid, "no recargar", reason);
						}
						else if (JB_Warnings [playerid]{NO_RELOAD} >= JB_Variables [NO_RELOAD] && (ivar != 26 || !JB_Variables [NO_RELOAD_SAWNOFF]))
						{
							GetWeaponName (ivar, reason, sizeof (reason));
							format (reason, sizeof (reason), "No recargar (%s)", reason);
							JB_Kick (playerid, reason);
							return 0;
						}
					}
			...
			JB_PlayerInfo [playerid][JB_pOldAmmo][ivar] = ammo;
			JB_PlayerInfo [playerid][JB_pLastWeaponUsed][ivar] = JB_PlayerInfo [playerid][JB_pLastUpdate];
			JB_PlayerInfo [playerid][JB_pOldWeapon] = ivar;
		}
	}
...
CALLBACK: OnPlayerDeath (playerid, killerid, reason)
{
	...
	if (killerid != INVALID_PLAYER_ID && !IsPlayerNPC (killerid) && (!JB_IsPlayerAdmin (killerid) || !JB_Variables [ADMIN_IMMUNITY]))
	{
		if (IsPlayerInRangeOfPlayer (killerid, 100.0, playerid))
		{
		    if (JB_PlayerInfo [killerid][JB_pCurrentState] == PLAYER_STATE_DRIVER)
		    {
				if (JB_Variables [DRIVE_BY] >= 2 && (reason == WEAPON_UZI || reason == WEAPON_MP5 || reason == WEAPON_TEC9 || reason == 50)) // 50 = helicopter blades.
				{
				    if (++JB_Warnings [killerid]{DRIVE_BY} >= MAX_CHECKS)
				        JB_Kick (killerid, "Excessive drive-by");
					else
					{
						JB_SetPlayerHealth (killerid, 0.0);
						SendClientMessage (killerid, JB_RED, "JunkBuster:{FFFF00} You have been killed for drive-by!");
					}
				}
				
			 	if (JB_Variables [ARMED_VEHICLES])
			 	{
			 	    new vehicleid = GetPlayerVehicleID (killerid);
					 	
					if (IsArmedVehicle (vehicleid))
					{
					    if (++JB_Warnings [killerid]{ARMED_VEHICLES} >= JB_Variables [ARMED_VEHICLES])
						{
						    new string [128];
								
						    format (string, sizeof (string), "Excessive killing with %s", JB_GetVehicleName (vehicleid));
						    JB_Kick (killerid, string);
						}
						else
							JB_SendFormattedMessage (killerid, JB_RED, "JunkBuster:{FFFF00} Killing with a %s is not allowed! Stop it! (Warning %d/%d)", JB_GetVehicleName (vehicleid), JB_Warnings [killerid]{ARMED_VEHICLES}, JB_Variables [ARMED_VEHICLES]);
					}
			 	}
			}
			...
			if (JB_Variables [SPAWNKILL])
			{
				if (JB_PlayerInfo [playerid][JB_pSpawnKillProtected])
				{
					if (++JB_Warnings [killerid]{SPAWNKILL} >= JB_Variables [SPAWNKILL])
						...
						JB_SendFormattedMessage (killerid, JB_RED, "JunkBuster:{FFFF00} Do not spawnkill! (Warning %d/%d)", JB_Warnings [killerid]{SPAWNKILL}, JB_Variables [SPAWNKILL]);
				}
		 	}
		}
...
{
    DOB::SetBit (JB_GunBought, playerid, false);
	SetSyncTime (playerid, SYNC_TYPE_POS);
	return 1;
}
...
CALLBACK: OnFilterScriptInit ()
{
	...
	if ((JB_AirbrakeDetection = JB_Variables [AIRBRAKE_DETECTION])) // start timer only when fast mode is activated.
	    SetTimer ("AirbrakeCheck", 337, true);
	
	//Cliente
    SendRconCommand("gamemodetext RolePlay v5.0");
	...
	JB_Log ("JunkBuster (v"#JB_VERSION") cargado correctamente.");
	JB_Log ("Nombres del servidor, cargado.");
    //SendRconCommand("hostname [ESP] SouthCenter — Juego de Rol");
    //Multi-nombre
    SetTimer("ServerDataUpdate",15000,1);
...
CALLBACK: OnFilterScriptExit ()
{
	...
	#endif
	if (JB_KickBanTitle != Text: INVALID_TEXT_DRAW)
		TextDrawDestroy (JB_KickBanTitle);
		
    if (JB_KickBanInfo != Text: INVALID_TEXT_DRAW)
		TextDrawDestroy (JB_KickBanInfo);
		
    if (JB_KickBanHelp != Text: INVALID_TEXT_DRAW)
		TextDrawDestroy (JB_KickBanHelp);
...
{
	GetPlayerName (playerid, JB_ReturnPlayerName(playerid), MAX_PLAYER_NAME);
	GetPlayerIp (playerid, JB_ReturnPlayerIp(playerid), 16);

	DOB::SetBit (JB_Freezed, playerid, false);
	DOB::SetBit (JB_KickBan, playerid, false);
	DOB::SetBit (JB_FullyConnected, playerid, false);
	DOB::SetBit (JB_AntiBugKilled, playerid, false);
	DOB::SetBit (JB_BuyingInAmmuNation, playerid, false);
	DOB::SetBit (JB_GunBought, playerid, false);
	...
	if (!IsPlayerNPC (playerid))
	{
	        ...
	        if (JB_Variables [BLACKLIST])
	        {
		        ...
		        JBDB::GetUserData (JB_ReturnPlayerName(playerid), banned, unbantime, whitelisted, ip);
		        if (banned)
		        {
		            new t = gettime ();
						
		            if (unbantime == 0 || !JB_Variables [TEMP_BANS])
		            {
		                SendClientMessage (playerid, JB_RED, ""COL_ADM"Administración:"COL_WHITE" Usted esta baneado en este servidor!");
						JB_LogEx ("%s (%s) has been banned for Ban evading.", JB_ReturnPlayerName(playerid), JB_ReturnPlayerIp(playerid));
						JBDB::UpdateUserData (JB_ReturnPlayerName(playerid), JB_ReturnPlayerIp(playerid), true, unbantime, whitelisted);
			            DOB::SetBit (JB_KickBan, playerid, true);
		                Kick (playerid);
			            return 0;
					}
					else if (t < unbantime)
					{
					    ...
					    JB_SendFormattedMessageToAll (JB_RED, ""COL_ADM"Administración:"COL_WHITE" Usted esta baneado temporalmente por %d día(s), %d hora(s), %d minuto(s) y %d segundo(s)!", days, hours, minutes, seconds);
						JB_LogEx ("%s (%s) has been banned for Ban evading (Temporary banned).", JB_ReturnPlayerName(playerid), JB_ReturnPlayerIp(playerid));
						JBDB::UpdateUserData (JB_ReturnPlayerName(playerid), JB_ReturnPlayerIp(playerid), true, unbantime, whitelisted);
			            DOB::SetBit (JB_KickBan, playerid, true);
		                Kick (playerid);
			            return 0;
					    ...
					    JBDB::UpdateUserData (JB_ReturnPlayerName(playerid), JB_ReturnPlayerIp(playerid), false, 0, whitelisted);
		        }
		        else if (JB_Variables [IP_BANS] && !whitelisted && JBDB::IsBanned (JB_ReturnPlayerName(playerid), JB_ReturnPlayerIp(playerid))) // Also checks for IP bans.
		        {
			...
			{
			    DOB::SetBit (JB_KickBan, playerid, true);
				SendClientMessage (playerid, JB_RED, ""COL_ADM"Administración:"COL_WHITE" Usted esta bloqueado en este servidor!");
				JB_LogEx ("%s (%s) has been banned for Ban evading.", JB_ReturnPlayerName(playerid), JB_ReturnPlayerIp(playerid));
	...
	JB_PlayerInfo [playerid][JB_pCurrentState] = 0;
	JB_PlayerInfo [playerid][JB_pLastVehicle] = 0;
	JB_PlayerInfo [playerid][JB_pFreezeTime] = 0;
	JB_PlayerInfo [playerid][JB_pLastKeyPressed] = GetTickCount ();
	JB_PlayerInfo [playerid][JB_pLastMoneyChange] = GetTickCount ();
	JB_PlayerInfo [playerid][JB_pLastLostMoney] = 0;
	for (new i = 0; i < MAX_FPS_INDEX; ++i)
		JB_PlayerInfo [playerid][JB_pFPS][i] = JB_Variables [MIN_FPS] + 1;

	if (JB_Variables [WARN_PLAYERS])
	{
		SendClientMessage (playerid, JB_GREEN_BLUE, "> Este servidor está ejecutando {FF0000}JunkBuster Anti-Cheat "#JB_VERSION"{00D799}!");
		SendClientMessage (playerid, JB_GREEN_BLUE, "> No puedes hacer trampas, de lo contrario te expulsarán o te banearan");
	}
	return 0;
}
...
CALLBACK: OnPlayerDisconnect (playerid, reason)
{
	DOB::SetBit (JB_FullyConnected, playerid, false);
	DOB::SetBit (JB_Spectating, playerid, false);
	if (DOB::GetBit (JB_KickBan, playerid))
		return 0;

    JBGMC::EndCheck (playerid, false);
	if (reason == 2 && JB_Variables [JB_CHROME])
...
CALLBACK: OnPlayerText (playerid, text [])
{
	...
	{
	    JB_LogEx ("El jugador '%s' probablemente ha escrito accidentalmente el comando RCON en el chat. Mensaje bloqueado. (\"%s\")", JB_ReturnPlayerName(playerid), text);
	    return 0;
	}

	if (JB_PlayerInfo [playerid][JB_pMuted] && JB_Variables [SPAM])
	{
		JB_SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} You are not allowed to chat for %d second(s) because you have been muted!", JB_PlayerInfo [playerid][JB_pMuted]);
		return 0;
	}
	...
	if (!JB_IsPlayerAdmin (playerid) || !JB_Variables [ADMIN_IMMUNITY])
	{
		...
		{
			if (!strcmp (text, JB_PlayerInfo [playerid][JB_pLastMessage], true, min (strlen (text), strlen (JB_PlayerInfo [playerid][JB_pLastMessage]))))
		...
		if (!JB_PlayerInfo [playerid][JB_pMuted] && JB_Variables [SPAM])
		{
			...
			{
				JB_PlayerInfo [playerid][JB_pMessageRepeated] = 0;
				JB_MutePlayer (playerid, 2 * 60, "Spam");
				return 0;
			}
		}
...
CALLBACK: OnPlayerCommandReceived (playerid, cmdtext [])
{
	...
	if (JB_Variables [COMMAND_SPAM] && (!JB_IsPlayerAdmin (playerid) || !JB_Variables [ADMIN_IMMUNITY]))
	{
		...
		{
			JB_Kick (playerid, "Extreme command spam");
			return 0;
		}
...
//==============================================================================
	...
	COMMAND:oputest (playerid, params [])
	{
		...
		t = GetTickCount () - t;
		JB_SendFormattedMessage (playerid, JB_RED, "OnPlayerUpdate time: %d ms", t);
		return 1;
	}
	...
	COMMAND:jbtest (playerid, params [])
	{
		...
		t = GetTickCount () - t;
		JB_SendFormattedMessage (playerid, JB_RED, "JunkBuster time: %d ms", t);
		return 1;
	}
...
COMMAND:fps (playerid, params [])
{
	...
	JB_SendFormattedMessage (playerid, JB_GREEN_BLUE, "Tus FPS: %d", JB_GetPlayerFPS (playerid));
	return 1;
}
...
COMMAND:gmctest (playerid, params [])//GodModeCheck test
{
	...
	if (JB_sscanf (params, "u", id))
		return SendClientMessage (playerid, JB_RED, "Usage:{D6D6D6} /gmctest <ID/name>");
	JB_GodModeCheck (id);
	JB_SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} Godmode Check realizado, si posee Godmode ver en /jbreportes", params);
	return 1;
}
...
COMMAND:jbcmds (playerid, params [])
{
	...
	new string [(sizeof (JB_AdminCommands)) * 32];
		
	for (new i = 0; i < sizeof (JB_AdminCommands); ++i)
		format (string, sizeof (string), "%s%s\n", string, JB_AdminCommands [i]);

	ShowPlayerDialog (playerid, DIALOG_CMDS, DIALOG_STYLE_LIST, "JunkBuster Comandos", string, "Usar", "Cerrar");
	return 1;
}
...
COMMAND:jbchrome (playerid, params [])
{
	...
	if (JunkBusterChrome (id, JB_ReturnPlayerName(playerid)))
		JB_SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} JunkBuster Chrome has successfully collected data of player '%s'!", JB_PlayerInfo  [id][JB_pName]);
	else
	    JB_SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} JunkBuster Chrome could not colect data of player '%s'!", params);
	return 1;
}
...
COMMAND:jbsethomepage (playerid, params [])
{
	...
	strcpy (JB_Homepage, params);
    DOF2::SetString (CONFIG_FILE, "Homepage", JB_Homepage, "Misc");
    JB_SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} Homepage has been set to '%s'!", params);
	return 1;
}

COMMAND:jbreportes (playerid, params [])
{
    #pragma unused params
	if (!JB_IsPlayerAdmin (playerid))
		return 0;
...
COMMAND:jbhackcodigos (playerid, params [])
{
    #pragma unused params
	if (!IsPlayerAdmin (playerid))
		return 0;

	ShowPlayerHackCodigoDialog (playerid);
	return 1;
}
...
COMMAND:jbsetvar (playerid, params [])
{
	...
	{
	    JB_SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} JunkBuster variable '%s' has successfully been set to %d.", var, value);
		JB_LogEx ("%s has set variable '%s' to %d.", JB_ReturnPlayerName(playerid), var, value);
	}
	return 1;
}
...
COMMAND:tban (playerid, params [])
{
	...
	if (JB_sscanf (params, "uiz", id, days, reason))
		return SendClientMessage (playerid, JB_RED, "Uso:{D6D6D6} /tban <ID> <dias> <razon>");

	if (IsPlayerConnected (id) && id != playerid && days > 0 && !isnull (reason))
	{
	    #if defined USE_DATABASE
	        JBDB::UpdateUserData (JB_PlayerInfo [id][JB_pName], JB_PlayerInfo [id][JB_pIp], true, gettime () + days * 60 * 60 * 24, false);
	        format (reason, sizeof (reason), "%s [%d día(s) baneados]", reason, days);
	        JB_Kick (id, reason);
	    #else
...
COMMAND:tunban (playerid, params [])
{
	...
	if (!isnull (params))
	{
		...
		#endif
				JB_SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} Temporary ban of player '%s' has successfully been deleted.", params);
			else
				JB_SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} Could not delete temporary ban of player '%s'!", params);
			return 1;
	}
	return SendClientMessage (playerid, JB_RED, "Uso:{D6D6D6} /tunban <IP>");
}
...
COMMAND:banip (playerid, params [])
{
	if (!IsPlayerAdmin (playerid))
		...
		if (!isnull (params))
		{
			...
			{
				if (BanIp (params))
					JB_SendFormattedMessage (playerid, JB_GREEN, ""COL_ADM"Administración:"COL_WHITE" IP %s fue baneada correctamente!", params);
				else
					JB_SendFormattedMessage (playerid, JB_RED, ""COL_ADM"Administración:"COL_WHITE" No se puede banear la IP %s!", params);
				return 1;
			}
		}
		return SendClientMessage (playerid, JB_RED, "Uso:{D6D6D6} /banIP <IP> ");
	#endif
}
...
COMMAND:unbanip (playerid, params [])
{
	if (!IsPlayerAdmin (playerid))
	    ...
	    if (JBDB::UnbanIp (params))
			JB_SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} IP %s has successfully been unbanned!", params);
		else
			JB_SendFormattedMessage (playerid, JB_RED, "JunkBuster:{FFFF00} Could not unban IP %s!", params);
		return 1;
	#else
		if (!isnull (params))
		{
...
{
	if (JB_Variables [SPECTATE_HACK] && newstate == PLAYER_STATE_SPECTATING && !JB_IsPlayerAdmin (playerid) && !IsPlayerNPC (playerid) && DOB::GetBit (JB_FullyConnected, playerid) && !DOB::GetBit (JB_Spectating, playerid))
		JB_Kick (playerid, "Spectate hack");
	...
	if (newstate == PLAYER_STATE_DRIVER)
	{
		...
		GetVehicleHealth (vehicleid, JB_PlayerInfo [playerid][JB_pVehicleHealth]);
		if (JB_PlayerInfo [playerid][JB_pVehicleHealth] > 1000.0) // Prevent innoncent players getting banned because of vehicle used by the cheaters
			SetVehicleHealth (vehicleid, 1000.0);
			
		if (JB_Variables [DRIVE_BY] >= 3)
        ...
        /*for (new j = 0; j < MAX_COMPONENT_SLOTS; ++j)
	    {
	        componentid = GetVehicleComponentInSlot (vehicleid, j);
	        if (componentid != 0 && componentid != (JB_VehicleComponents [vehicleid]{j} + 999))
	            JB_RemoveVehicleComponent (vehicleid, componentid);
	    }*/
			
        JB_PlayerInfo [playerid][JB_pLastVehicle] = vehicleid;
		if (JB_PlayerInfo [playerid][JB_pVehicleEntered] != vehicleid && !JB_IsPlayerAdmin (playerid) && !IsPlayerNPC (playerid))
		{
			...
			if (driver != INVALID_PLAYER_ID)
			{
				...
				if (JB_Variables [CAR_JACK_HACK] && JB_Warnings [playerid]{CAR_JACK_HACK} >= MAX_CHECKS)
					JB_Kick (playerid, "Carjack hack");
				else
					JunkBusterReport (playerid, "carjack hack", JB_PlayerInfo [driver][JB_pName]);
			}
			else if (oldstate == PLAYER_STATE_ONFOOT)
			{
			    if (!IsPlayerInRangeOfPoint (playerid, 20.0, JB_PlayerInfo [playerid][JB_pOnFootPos][0], JB_PlayerInfo [playerid][JB_pOnFootPos][1], JB_PlayerInfo [playerid][JB_pOnFootPos][2]))
			    {
			        JB_Warnings [playerid]{VEHICLE_TELEPORT}++;
			        if (JB_Variables [VEHICLE_TELEPORT] && JB_Warnings [playerid]{VEHICLE_TELEPORT} >= MAX_CHECKS)
						JB_Kick (playerid, "Vehicle teleport");
			    }
			}
		}
	...
	{
	    JB_PlayerInfo [playerid][JB_pVehicleHealth] = 1000.0;
	    if (newstate == PLAYER_STATE_PASSENGER && JB_Variables [DRIVE_BY] >= 4)
			SetPlayerArmedWeapon (playerid, 0);
		JB_PlayerInfo [playerid][JB_pLastVehicle] = 0;
	}
	...
	JB_PlayerInfo [playerid][JB_pVehicleEntered] = INVALID_VEHICLE_ID;
	JB_PlayerInfo [playerid][JB_pAirbraking] = 0;
	JB_PlayerInfo [playerid][JB_pWallriding] = 0;
	JB_PlayerInfo [playerid][JB_pOldSpeed] = 0;
	JB_PlayerInfo [playerid][JB_pLastAirbrakeSpeed] = 0;
	GetPlayerPos (playerid, JB_PlayerInfo [playerid][JB_pOldAirbrakePos][0], JB_PlayerInfo [playerid][JB_pOldAirbrakePos][1], JB_PlayerInfo [playerid][JB_pOldAirbrakePos][2]);
	return 1;
}
...
CALLBACK: OnPlayerKeyStateChange (playerid, newkeys, oldkeys)
{
    ...
    JB_Warnings [playerid]{AFK} = 0;
	JB_PlayerInfo [playerid][JB_pLastKeyPressed] = GetTickCount ();
	
	if (!JB_IsPlayerAdmin (playerid) || !JB_Variables [ADMIN_IMMUNITY])
	{
		if (JB_Variables [CBUG] && JB_PlayerInfo [playerid][JB_pCurrentState] == PLAYER_STATE_ONFOOT)
		{
			switch (GetPlayerWeapon (playerid))
			{
				case 24, 25, 27, 29, 30, 31, 33, 34: // Deagle, Shotung, SPAS12, MP5, AK47, M4, Rifle, Sniper Rifle
				{
					...
					{
						if (++JB_Warnings [playerid]{CBUG} & 1 == 0)
							SendClientMessage (playerid, JB_RED, ""COL_ADM"Administración:"COL_WHITE" Por favor, deje de realizar el C-Bug o será expulsado/baneado.");
	...
	if (1 <= JB_Variables [DRIVE_BY] <= 2 && PRESSED (KEY_FIRE) && (HOLDING (KEY_LOOK_RIGHT) || HOLDING (KEY_LOOK_LEFT)))
	{
		if (!IsPlayerInPlane (playerid) && GetVehicleModel (GetPlayerVehicleID (playerid)) != 432)//Ignore planes and rhinos.
		{
			...
			if (weaponid != 0 && ammo != 0)//Check if player has got an SMG. Don't punish innoncent players.
			{
				...
				TogglePlayerControllable (playerid, false);
				JB_PlayerInfo [playerid][JB_pFreezeTime] = 3;
				SetSyncTime (playerid, SYNC_TYPE_VEHICLE);
			}
		}
	}
				...
				if (response)
				{
					switch (listitem)
					{
						...
						case 3:
						{
	    					...
	    					JB_Log ("JunkBuster variables have been reset to default values.");
							ShowPlayerConfigDialog (playerid);
						}
			...
			case DIALOG_SETVAR .. (DIALOG_SETVAR + MAX_JB_VARIABLES - 1):
			{
				if (response)
				{
					...
					else
					{
						...
						JB_SendFormattedMessage (playerid, JB_GREEN, "JunkBuster:{FFFF00} JunkBuster variable '%s' has successfully been set to %d.", JB_VariableNames [var], JB_Variables [var]);
...
CALLBACK: OnRconCommand (cmd [])
{
	...
	{
	    if (SetJBVar (var, value))
	        JB_LogEx ("RCON admin has set variable '%s' to %d.", var, value);
		return 1;
	}
	...
	{
	    JB_Variables = JB_DefaultVariables;
	    JB_Log ("JunkBuster variables have been reset to default values by an RCON admin.");
	    return 1;
	}
...
CALLBACK: OnPlayerEnterRaceCheckpoint (playerid)
{
	...
	if (!JB_SyncInfo [playerid][SYNC_TYPE_POS][JB_sSyncTime] && !JB_IsPlayerAdmin (playerid))
	{
		if (JB_Variables [CHECKPOINT_TELEPORT])
		{
			if (!JB_GetPlayerSpeed (playerid, false))
			{
				...
				if (JB_Warnings [playerid]{CHECKPOINT_TELEPORT} >= MAX_CHECKS)
					JB_Kick (playerid, "Checkpoint teleport");
				else
				{
					new string [128];
...
PRIVATE: JB_sscanf (string [], format [], {Float, _}: ...)
{
	#if defined isnull
		if (isnull (string))
	#else
